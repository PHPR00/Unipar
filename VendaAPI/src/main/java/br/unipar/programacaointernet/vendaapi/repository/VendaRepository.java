...
import br.unipar.programacaointernet.vendaapi.dto.ClienteTotalVendaDTO;
...
public class VendaRepository {
    ...
    public List<Venda> buscarPorValorTotalCliente(BigDecimal valor) {
        String jpql = "SELECT v FROM Venda v INNER JOIN v.cliente c WHERE v.total = :valor";
        return em.createQuery(jpql, Venda.class).setParameter("valor", valor).getResultList();
    }

    public List<ClienteTotalVendaDTO> qtdVenda() throws Exception{
        try {
            String jpql =
            ...
            List<ClienteTotalVendaDTO> quantidadeDTOList = new ArrayList<>();

            for (Object[] objects : listaObjeto) {
                String nome = (String) objects[0];
                Long quantidadeVenda = (Long) objects[1];

                quantidadeDTOList.add(new ClienteTotalVendaDTO(nome, quantidadeVenda));
            }
