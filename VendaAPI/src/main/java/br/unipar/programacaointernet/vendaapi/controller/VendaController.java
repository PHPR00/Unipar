...
public class VendaController {
    ...
    @GET
    @Path("/valorTotalCliente")
    @Produces(value = MediaType.APPLICATION_JSON)
    public Response listarValorTotalCliente() {
        List<ValorTotalClienteDTO> vendas = produtoService.listarProdrutoDescricao();
        if(vendas.isEmpty())
            return Response.status(204).entity("Nenhuma venda encontrada com o valor total especificado").build();
    ...
    @GET
    @Path("/quantidadeVenda")
    @Produces(value = MediaType.APPLICATION_JSON)
    public Response listarQuantidadeVenda() throws Exception {
        List<ClienteTotalVendaDTO> vendas = vendaService.dtoList();
        if(vendas.isEmpty())
            return Response.status(204).entity("Nenhuma venda encontrada com o valor total especificado").build();
